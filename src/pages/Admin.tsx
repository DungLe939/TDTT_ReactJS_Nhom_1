...
export const Admin = () => {
  ...
  const [activeTab, setActiveTab] = useState<'add-dish' | 'add-info'>('add-dish');
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
        ...
        <p className="text-neutral-600 max-w-md">
          Xin lỗi, trang này chỉ dành cho tài khoản có quyền Quản trị viên (Admin). Vui lòng đăng nhập bằng tài khoản Admin để truy cập.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex gap-4 border-b border-neutral-200">
        <button
          ...
          className={`pb-4 px-4 font-medium text-sm transition-colors relative ${activeTab === 'add-dish' ? 'text-indigo-600' : 'text-neutral-500 hover:text-neutral-700'
            }`}
        >
          <span className="flex items-center gap-2"><PlusCircle className="w-4 h-4" /> Thêm món ăn mới</span>
          {activeTab === 'add-dish' && (
        ...
        <button
          ...
          className={`pb-4 px-4 font-medium text-sm transition-colors relative ${activeTab === 'add-info' ? 'text-indigo-600' : 'text-neutral-500 hover:text-neutral-700'
            }`}
        >
          <span className="flex items-center gap-2"><Info className="w-4 h-4" /> Thêm thông tin AI</span>
          {activeTab === 'add-info' && (
            <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600" />
          )}
        </button>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      {...register('region', { required: 'Vui lòng nhập xuất xứ' })}
